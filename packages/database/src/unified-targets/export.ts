 ...
 *
 * Library entry point: `exportUnifiedTargets(db, format, tier?)`.
...
/**
 * Read `unified_targets` (optionally filtered by tier) and render the feed
 * body: CSV text for `"csv"`, pretty-printed JSON for `"json"`.
 */
export async function exportUnifiedTargets(
  ...
  const where = tier === undefined || tier === null ? "" : "WHERE tier = $1";
  const params = tier === undefined || tier === null ? [] : [tier];
  const { rows } = await query(
    `SELECT ${EXPORT_COLUMNS.join(", ")} FROM unified_targets ${where} ORDER BY company_name`,
    params,
  );
  if (format === "json") return `${JSON.stringify(rows, null, 2)}\n`;
  return stringifyUnifiedCsv(rows);
}
